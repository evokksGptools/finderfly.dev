...
$(document).ready(function(){
	...
	if(hwidth >= 1920){
		var li = document.getElementById("li-entrar");
		var bt = document.getElementById("bt-entrar");
		li.removeChild(bt);
		var header = document.getElementById("header-button");
		$('.4c').css({"border":"none"});
		$(bt).removeClass("bt-lg-nav");
		$(bt).addClass("bt-lg-header");
	...
	window.onresize=function(){
		...
		if(hwidth <= 1450){
			var header = document.getElementById("header-button");
			var bt = document.getElementById("bt-entrar");
			header.removeChild(bt);
			var li = document.getElementById("li-entrar");
			$('.4c').css({"border-right":"1px solid #FFF"});
			$(bt).removeClass("bt-lg-header");
			$(bt).addClass("bt-lg-nav");
			li.appendChild(bt);
		}
	};
});
...
function enviaCadastro(url, destino){
	...
	//Campos do formulario
	var nome = document.forms["form-cad"]["nomecad"].value;
	var email = document.forms["form-cad"]["emailcad"].value;

	var camposCad = "nomecad="+nome+"&emailcad="+email;
	...
	if(validate){
		...
		enviaCad.send(camposCad);
	}
}

function loginCake(url, destino){
	//Validação do formulario
	var validate = validateFormLogin();

	//Campos do formulario
	var user = document.forms["form-login"]["u"].value;
	var pass = document.forms["form-login"]["p"].value;
	var ip = document.forms["form-login"]["ip_address"].value;

	var camposLogin = "username="+user+"&password="+pass.replace("#","%23")+"&ip_address="+ip;
	var dados = new FormData();
	dados.append('username',user);
	dados.append('password',pass);
	dados.append('ip_address',ip);
		...
		enviaCad.onreadystatechange = function(){
			if(enviaCad.readyState == 4 || enviaCad.readyState ==0){
				if(enviaCad.status ==200){
					...
					var obj = JSON.parse(enviaCad.responseText);

					if(obj.status == true){
						var form = document.getElementById('form-login-cake');
						form.setAttribute('action',"http://pricefindercp.com.br/advertisers/login.ashx?tp=1");
						form.submit();

					}else{

					}
					document.getElementById("resposta_ajax_lg").innerHTML = obj.mensagem;
					setInterval(function(){elemento.removeAttribute("class","d_back_box")}, 2000);
					limpaCamposLogin();
				}
				else{
